...
public class TemplateFormatStrategyTest extends TestCase {
    ...
    public void testMappedTypes() {
        ...
        assertEquals(JUnitHelper.dateString(1972, 6, 20), formatStrategy.format(null, new java.sql.Date(
                new GregorianCalendar(1972, 6, 20).getTime().getTime())));
        assertEquals(JUnitHelper.dateString(1972, 6, 20, 2, 15, 4), formatStrategy.format(null, new java.sql.Timestamp(
                new GregorianCalendar(1972, 6, 20, 2, 15, 4).getTime().getTime())));
        assertEquals(JUnitHelper.dateString(1972, 6, 20), formatStrategy.format(null,
                new GregorianCalendar(1972, 6, 20).getTime()));
    }
